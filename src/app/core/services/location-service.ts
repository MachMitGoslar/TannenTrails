...
export class LocationService {
  ...
  public permissionGranted$: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(false);
  public my_position$: BehaviorSubject<Position | null> =
    new BehaviorSubject<Position | null>(null);
  ...
  checkPermissionStatus() {
    if (!this.platform.is('hybrid')) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          console.error(
            'Error obtaining position for permission check:',
            error
          );
